...
The parser takes the same `(doc_id, "parsed")` key for the entire duration it writes or rewrites a document's `*.parsed/` sidecar directory, on every parse attempt, not only retries. This is what prevents a torn read of the `parsed` kind: a retry that reuses an existing `*.parsed/` directory in place (§Retention of the archived source is deliberate) rewrites it under the same lock a concurrent export build must also acquire before reading (§Build task, step 2), so a build either reads the complete prior output (if it wins the race) or waits and reads the complete new output (if it loses it) — never a mix of old and new files. This guarantee does not depend on whether the parser writes files into the directory incrementally or stages-then-publishes; the lock alone is sufficient regardless of the parser's internal write strategy. During a document's first-ever parse this lock is never contended, because no `sidecar_location` is published (and so no export can pass admission) until that parse succeeds — see §Availability and half-processed content.
...
- `source`: available whenever `doc_status.metadata.source_location` is persisted and resolves to an existing regular file. This is true from shortly after enqueue onward, because the RFC's own model guarantees the source bytes never change in place — only their location does, which the per-artifact lock already governs (§Source-location lifecycle and orphan rotation, step 11).
- `parsed`: available whenever `full_docs.sidecar_location` is persisted for that `doc_id` — nothing else. `doc_status`'s current processing stage is deliberately **not** part of this predicate: `sidecar_location` is already "the only authority for the parsed artifact" (§Artifact identity fields), and gating on `doc_status` in addition would duplicate that authority and could disagree with it. Before a document's first successful parse, no `sidecar_location` exists yet, so this check alone already returns "unavailable" (`404`) — there is nothing yet to protect, so there is no race to resolve.

The actual guarantee against downloading half-processed content is not this check — it is the per-artifact lock (§Per-artifact keyed lock), enforced at build time, after admission. If a retry is rewriting a document's `*.parsed/` directory in place at the moment a client requests the `parsed` artifact, or at the moment an admitted build starts reading it, exactly one of two things happens:
...
There is no interleaving between these two outcomes: the request-time availability check only decides whether a job is worth creating at all, while the lock — taken unconditionally by both sides before either reads or writes — is what makes a torn, half-written read impossible regardless of what the advisory check observed a moment earlier. A build that cannot acquire the lock within its own `MAX_DOWNLOAD_PREPARE_SECONDS` budget fails the job with a sanitized `artifact_busy` error rather than blocking indefinitely; the client may simply retry the export request. The same reasoning that rules out a torn read of the *parsed* directory also confirms there is no equivalent race for `source`: nothing ever rewrites the source file's bytes in place, so its only failure mode under concurrent pipeline activity is a stale-locator read, which the build's "strictly re-resolve fresh from storage" step (§Build task, step 3) and the lock together already rule out.
