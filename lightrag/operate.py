...
async def _get_vector_context(
    ...
    if not results:
        ...
        if explicit_chunks:
            ...
            return _structured_rank(query, explicit_chunks)
        return []
    ...
    if explicit_chunks:
        ...
        return _structured_rank(query, explicit_chunks + deduped)
    return _structured_rank(query, valid_chunks)


_EXPLICIT_ID_RE = re.compile(r"\b(?:FACT|EQ|REF|TBL|FIG)(?:-[A-Z0-9]+)*\d{2,}\b")
...
def _structured_candidate_type(chunk: dict) -> str:
...
def _structured_overlap(query: str, chunk: dict) -> int:
    query_tokens = {
        token.lower()
...
def _structured_rank(query: str, chunks: list[dict]) -> list[dict]:
    """Apply explicit structural constraints before dense ordering.

    R1 experiment.  Query identifiers are not treated merely as extra recall
    keys; they become ranking tiers.  Within the same tier, lexical overlap
    with the query is a cheap proxy for semantic row ranking until a real
    cross-encoder is evaluated.
    """
