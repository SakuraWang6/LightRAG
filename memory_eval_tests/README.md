...
KG 抽取在本次专属的隔离子服务中执行，并带有固定的完整性保护：使用 JSON 结构化输出、单并发、1200 秒抽取超时、32K 上下文窗口与 16K 输出预算，单次响应上限 40 条记录与 16 个实体；初次输出仍被截断时不进行续写补全，直接判定文档失败，避免索引不完整或不可靠的知识图谱。这些值会随运行参数写入 `run.json` 的参数快照。
