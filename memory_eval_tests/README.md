...
4. **检索评分**：对非拒答题调用 `/query/data`，按证据内容（答案文本、数值/单位、公式与表格行）在有序 chunk 中逐条匹配；FACT 编号单独出现不再计为命中。计算证据召回、MRR 和上下文精确率，并记录每条命中证据的聚焦片段与完整 chunk。
...
KG 抽取在本次专属的隔离子服务中执行，并带有固定的完整性保护：使用 JSON 结构化输出、单并发、1800 秒抽取超时、32K 上下文窗口与 16K 输出预算，单次响应上限 40 条记录与 16 个实体；初次输出仍被截断时不进行续写补全，直接判定文档失败，避免索引不完整或不可靠的知识图谱。这些值会随运行参数写入 `run.json` 的参数快照。
...
4. 完成后查看结果摘要、逐题详情、报告、日志、导入回执和失败归因；逐题详情中会显示每条证据命中的聚焦片段（含位置、匹配类型和完整 chunk），并在模型达到输出上限时显示“回答可能被截断”的警告；
5. 仅在不再需要复核时删除运行；删除会同时清除该运行的隔离索引和记录。

WebUI 和 CLI 使用相同的 `memory_eval_tests.cli` 入口及运行信封，因此两者结果可以并列查看。

运行测评与数据集生成使用独立队列：运行测评默认一次执行一个，以保护本地 Ollama；数据集生成不会等待运行测评。数据集生成默认也一次一个，以避免多个 DOCX 写入任务争用本机资源；如需提高该容量，可设置 `MEMORY_EVAL_MAX_ACTIVE_DATASET_JOBS`。运行测评的容量仍由 `MEMORY_EVAL_MAX_ACTIVE_JOBS` 控制。
...
| `case_trace.json` | 每道题的 oracle、检索结果（含聚焦证据片段与完整 chunk）、回答（含 provider 截断信号）和最终上下文追踪 |
...
- **证据召回@K（`average_recall`）**：oracle 所需事实被检索到的比例；低值通常意味着解析、索引或检索问题。证据命中按内容判定（答案文本、数值/单位、公式与表格行需实际出现在检索 chunk 中），FACT 编号本身不构成命中证据。
...
- **生成截断率（`generation_truncation_rate`）**：模型因输出 token 上限而返回不完整回答的比例；仅在 provider 提供截断信号时统计。
...
/Users/sakura/miniconda3/envs/lightrag-memory-eval/bin/python3.11 -m pytest -q \
  tests/memory_eval/test_answer.py \
  tests/memory_eval/test_product_evaluation.py \
  tests/memory_eval/test_retrieval.py \
  tests/api/routes/test_eval_jobs.py \
  tests/api/routes/test_eval_routes.py
```

这些测试覆盖单一运行信封、原子 JSON 发布、产品 CLI 命令构造、回答截断信号与内容级证据匹配的回归，以及 API 对已移除字段和基础设施参数的拒绝。
