...
def _aggregate(rows: list[dict[str, Any]]) -> dict[str, Any]:
    total = len(rows)
    metric = lambda key: (
        sum(bool(row.get(key)) for row in rows) / total if total else 0.0
    )
    ...
    for name in ("FACT", "TABLE", "FIGURE", "FORMULA", "MULTIHOP", "ABSTAIN"):
        ...
        if subset:
            grouped[name] = {
                "cases": len(subset),
                "answer_accuracy": metric_for(subset, "exact_match"),
                "groundedness": metric_for(subset, "grounded"),
            }
    return {
        "cases": total,
        ...
        "candidate_recall": sum(row["candidate_recall"] for row in rows) / total
        if total
        else 0.0,
        "selected_recall": sum(row["selected_recall"] for row in rows) / total
        if total
        else 0.0,
        "selection_precision": _average(rows, "selection_precision"),
        "mean_candidate_context_chars": sum(
            row["candidate_context_chars"] for row in rows
        )
        / total
        if total
        else 0.0,
        "mean_selected_context_chars": sum(
            row["selected_context_chars"] for row in rows
        )
        / total
        if total
        else 0.0,
        "mean_evidence_count": sum(row["evidence_count"] for row in rows) / total
        if total
        else 0.0,
...
def _render_report(payload: dict[str, Any]) -> str:
    ...
    for item in payload["methods"]:
        ...
        selected_k = (
            "≤" + str(item["selected_limit"])
            if item["selector"]
            else str(item["selected_limit"])
        )
        lines.append(
    ...
    for item in payload["methods"]:
        lines.extend(
            [
                f"### {item['label']}",
                "",
                "| Type | Cases | Accuracy | Groundedness |",
                "|---|---:|---:|---:|",
            ]
        )
        for name, row in item["summary"]["by_question_type"].items():
            lines.append(
                f"| {name} | {row['cases']} | {row['answer_accuracy']:.4f} | {row['groundedness']:.4f} |"
            )
        lines.append("")
    lines.extend(
        [
            "## Reproducibility",
            "",
            "- Historical KG storage and cached keywords were reused; query-cache writes were disabled.",
            "- Raw selector outputs and raw answers are retained in the JSON artifact for each question.",
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    complete_ids = {
        row["question_id"] for method, _, _ in METHODS for row in method_rows[method]
    }
    completed_all_methods = {
        question_id
        for question_id in complete_ids
        if all(
            question_id in {row["question_id"] for row in method_rows[method]}
    ...
    def current_payload() -> dict[str, Any]:
        ...
        return {
            ...
            "status": "complete"
            if len(completed_all_methods) == len(questions)
            else "in_progress",
            "methods": methods,
        }
        ...
        for index, question in enumerate(questions, start=1):
            ...
            for method, label, selected_limit in METHODS:
                ...
                matched_candidate_facts = [
                    ...
                    if any(
                        contains_fact(candidate, fact) for candidate in candidate_pool
                    )
                ]
                ...
                method_rows[method].append(
                    {
                        ...
                        "candidate_recall": len(matched_candidate_facts) / denominator
                        if denominator
                        else 1.0,
                        "selected_recall": len(matched_selected_facts) / denominator
                        if denominator
                        else 1.0,
                        ...
                        "evidence_retention_rate": len(selected_ids)
                        / len(candidate_pool),
                        ...
                        "evidence_density_proxy": len(relevant_selected)
                        / len(selected_ids)
                        if selected_ids and denominator
                        else None,
