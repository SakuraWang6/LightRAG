...
def _report_markdown(
    ...
    lines = ["# 测评报告", "", "## 结果概览", "", "| 指标 | 结果 |", "| --- | --- |"]
    lines.append(f"| 正确题数 / 总题数 | {correct} / {total} |")
    lines.append(f"| 回答准确率 | {_format_rate(answer.get('answer_accuracy'))} |")
    lines.append(f"| 证据支撑率 | {_format_rate(answer.get('groundedness'))} |")
    lines.append(f"| 未通过题数 | {len(failed)} |")
    if uncertain:
    ...
    if not failed:
        ...
        lines.append("")
        lines.append(
            "归因基于最终模型可见上下文与检索候选的确定性判定；"
            "每题的证据片段与完整上下文可在“逐题详情”中查看。"
        )
    ...
    if failed:
        ...
        for row in failed:
            qid = str(row.get("question_id") or "")
            cause = cause_by_qid.get(qid, "unclassified")
            expected = str(row.get("expected") or "")[:90]
            actual = str(row.get("answer") or "").replace("\n", " ")[:140]
            lines.append(
                f"| {qid} | {row.get('question_type') or '—'} | "
                f"{_CAUSE_LABELS.get(cause, cause)} | {expected} | {actual} |"
            )
    return "\n".join(lines)
